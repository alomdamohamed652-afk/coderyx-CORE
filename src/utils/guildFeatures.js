...
function snapshot(guildId) {
  const out = {};
  for (const [name, meta] of Object.entries(featureCatalog)) out[name] = isEnabled(guildId, name, Boolean(meta.defaultEnabled));
  const configured = guildConfig.getAll(guildId);
  for (const name of Object.keys(configured)) if (name.startsWith("feature.")) out[name.slice(8)] = Boolean(configured[name]);
  return out;
}
